...
from .util import (
    TextProcessor, DocumentMetadata, SearchEngine,
    create_document_metadata, ValidationUtils
)
...
class RecommendationEngine:
        ...
        try:
            ...
            return RecommendationResponse(
                ...
                metadata={
                    ...
                    'recommendation_types': list(set(r.recommendation_type for r in recommendations))
                }
            )
    ...
    async def _get_content_similarity_recommendations(self, query: str, limit: int) -> List[Recommendation]:
            ...
            for heading in doc_info['headings']:
                for term in query_terms:
                    if term in heading['title'].lower():
                        heading_bonus += 5 / heading['level']  # Higher level headings get more bonus
    ...
    async def _get_popular_recommendations(self, query: str, limit: int) -> List[Recommendation]:
        ...
        for doc_path in self.indexer.get_all_documents():
            ...
            for pattern, description, points in important_patterns:
                if re.search(pattern, doc_path.lower()) or re.search(pattern, doc_info['title'].lower()):
                    score += points
            ...
            query_relevant = any(term in doc_info['title'].lower() or 
                               term in doc_info['description'].lower() 
    ...
    async def _get_related_api_recommendations(self, query: str, limit: int) -> List[Recommendation]:
...
# Import math for TF-IDF calculations
import math