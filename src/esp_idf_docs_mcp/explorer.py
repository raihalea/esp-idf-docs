...
from .util import (
    TextProcessor, SearchEngine, FileCache, ValidationUtils,
    DocumentAnalyzer, SearchConfig, DocumentMetadata,
...
class ESPIDFDocsExplorer:
    ...
    def _validate_file_path(self, file_path: str) -> None:
        ...
        if path_obj.suffix not in self.config.allowed_extensions:
            raise ValueError(f"File must have one of these extensions: {self.config.allowed_extensions}")
    ...
    async def search_docs(self, query: str, limit: Optional[int] = None, 
        ...
        try:
            ...
            logger.info(f"Starting search for query: '{query}' (limit={effective_limit}, offset={offset})")
            ...
            logger.info(f"Search completed: found {len(all_results)} results in {files_scanned} files "
                       f"({search_time:.2f}ms)")
    ...
    async def read_doc(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if file_size > self.config.max_file_size_kb * 1024:
                raise ValueError(f"File too large: {file_size} bytes (max: {self.config.max_file_size_kb}KB)")
            ...
            for file_path in all_files:
                ...
                try:
                    ...
                    for api_pattern, match_type in api_patterns:
                        matches = list(re.finditer(api_pattern, content, re.IGNORECASE | re.MULTILINE))
